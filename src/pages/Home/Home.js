...
import UpdateDsk from "../../assets/images/NIOB-Update-Banner-2.jpg";
...
const Home = () => {
  ...
  const [niobTotalSupply, setNiobTotalSupply] = useState(0);
  const [niobBusdValue, setNiobBusdValue] = useState(0);
  const [totalMinted, setTotalMinted] = useState(0);
  const [totalLockedRewards, setTotalLockedRewards] = useState(0);
  const [niobBnbWorth, setNiobBnbWorth] = useState(0);
  const [niobBusdWorth, setNiobBusdWorth] = useState(0);
      ...
      try {
        ...
        for (let i = 0; i < pL; i++) {
          ...
          if (poolInfo.lpToken != undefined) {
            ...
            const reserve = await ExchangeService.getReserves(NIOB_BUSD_LP);
            const tokenZero = await ExchangeService.getTokenZero(
              NIOB_BUSD_LP
            );
            const tokenOne = await ExchangeService.getTokenOne(NIOB_BUSD_LP);
            ...
            const rewards = Number(
              Number(
                (await FarmService.pendingNiob(i, isUserConnected)) /
                  10 ** 18
              ).toFixed(3)
            );
    ...
    if (liquidity != 0) {
      const apr =
        ((allocPoint / totalAllcationPoint) *
          ((niobPerBlock / 10 ** 18) * 28800 * 365 * 100 * niobPrice)) /
        liquidity;
  ...
  const getTotalSupply = async () => {
    const res = await ExchangeService.getTotalSupply(
      MAIN_CONTRACT_LIST.niobNew.address
    );
    const niobTotalSupply = res;
    const txAmount = (0.05 * niobTotalSupply) / 100;
    setTotalMinted(niobTotalSupply);
    setNiobTotalSupply(txAmount);
    return res;
  };
        ...
        <Row className="startEngine">
          <Col xl={4} lg={12}>
            <div className="comnBlk">
              ...
              <h3>NIOB Price: ${niobBusdValue.toFixed(2)}</h3>
              <Link
                to={"/trade/exchange"}
                className="captionFooter d-flex justify-content-between align-items-center"
              >
            ...
            <div className="comnBlk mb-4">
              ...
              <ul className="pl-0 niobStats">
                ...
                <li>
                  ...
                  <span>{addCommas(niobTotalSupply.toFixed(2))}</span>
                </li>
                <li>
                  <label>New Niob/Block</label>
                  <span>50</span>
                  {/* <span>{addCommas(niobPerBlock)}</span> */}
                </li>
                <li>
