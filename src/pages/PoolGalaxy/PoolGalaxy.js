      ...
      for (let i = 0; i < pL; i++) {
        ...
        if (poolInfo) {
          if (Number(poolInfo.allocPoint) === 0) {
            setInactiveFarms(inactiveFarms => [...inactiveFarms, { poolInfo, userInfo, pid: i, isLocked: niobId == i ? true : false  }]);
          } else {
            if (Number(userInfo.amount) > 0) {
              // stakingOnlyArrayToSort.push({ poolInfo, userInfo, pid: i, niobId })
              setStakingOnly(stakingOnly => [...stakingOnly, { poolInfo, userInfo, pid: i, isLocked: false }]);
            }
            // activePoolsToSort.push({ poolInfo, userInfo, pid: i, niobId })
            setFarms(farms => [...farms, { poolInfo, userInfo, pid: i, niobId, isLocked: false }]);
          }
        }
      }
