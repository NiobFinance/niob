...
const PlanetCard = (props) => {
      ...
      if (lpToken) {
        ...
        if (isUserConnected) {
          ...
          const rewards = Number(
            Number(
              (await FarmService.pendingNiob(pid, isUserConnected)) /
              10 ** 18
            ).toFixed(5)
          );
          if (!check && amount > 0) {
  ...
  const calPrice = async (pairAddress) => {
    // console.log('hootttt');
    // console.log('dddddd',pairAddress);
    let price = 0;
      ...
      >
        ...
        <div className="coin_detail">
          ...
          <div className="erndniob">
            ...
            <p>$ {earnedDollarValue(dollarValue, stakeAmounts.rewards) === "NaN" || NaN ? 0 : earnedDollarValue(dollarValue, stakeAmounts.rewards)}</p>
          </div>
        </div>
